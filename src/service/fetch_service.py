    ...
    async def _fetch_with_custom_strategies_order(self, request: FetchRequest, strategies: list[FetchStrategy]) -> FetchResult:
        for strategy in strategies:
            try:
                ...
                needs_js = self._js_detection_service.needs_javascript(result.html)
                if needs_js.needs_js and strategies.index(strategy) != len(strategies) - 1: # only go to next fetcher if there are fetchers left
                    logger.info(f"Result indicates JS is required: {needs_js.reason} ({needs_js.matched_html if needs_js.matched_html else needs_js.body[:100]}...). Skipping {strategy.value}")
                elif result.status_code in _REJECTION_CODES:
